...
public class PdsInputOkCommand implements CommonInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int MaxSize = 1024*1024*30; //서버에 저장시킬 최대 파일 용량을 30MByte로 제한
		String encoding = "UTF-8";
		
		MultipartRequest multipartRequest = new MultipartRequest(request, realPath, MaxSize, encoding, new  DefaultFileRenamePolicy()); 
		System.out.println("INPUTOKCOMMAND MULTI");
		...
		oFileName.substring(0, oFileName.length()-1);
		fsName.substring(0, fsName.length()-1);
		
		//파일 업로드시킨 정보를 DB에 저장하기 위한 준비...
