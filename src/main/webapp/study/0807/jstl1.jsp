...
<div class="container">
  <h2>JSTL(Java Standard Tag Library)</h2>
	...
	<hr/>
	<div>
		앞의 정의된 라이브러리를 사용할 경우에는 jsp지시자중 'taglib'를 이용하여 먼저 선언후 사용한다.
	</div>
	<hr/>
	<h2>Core 라이브러리 사용예</h2>	
		...
		su1변수 값을 출력1? <c:out value="${su1}"></c:out><br/>
		su1변수 값을 출력2? ${su1}<br/>
		...
		<div>3. su3 > su4 : <c:if test="${su3+'0'>su4+'0'}" >3. su3는 su4보다 크다.</c:if></div>
		<div>4. su3 <= su4 : <c:if test="${su3+'0'<=su4+'0'}" >4. su3는 su4보다 작거나 같다.</c:if></div>
		<div>5. su3 < su4 : <c:if test="${su3<su4}" >5. su4는 su3보다 크다.</c:if></div>
		<div>6. su3 >= su4 : <c:if test="${su3>=su4}" >6. su4는 su3보다 작거나 같다.</c:if></div>
		<div>6. su3 >= su4 : <c:if test="${su3>=su4}" >6. su4는 su3보다 작거나 같다.</c:if></div>
		<div>3_1. su3 gt su4 : <c:if test="${su3 gt su4}" >6. su4는 su3보다 작거나 같다.</c:if></div>
		<div>4_1. su3 le su4 : <c:if test="${su3 le su4}" >4. su3는 su4보다 작거나 같다.</c:if></div>
		<div>5_1. su3 lt su4 : <c:if test="${su3 lt su4}" >5. su4는 su3보다 크다.</c:if></div>
		<div>6_1. su3 ge su4 : <c:if test="${su3 ge su4}" >6. su4는 su3보다 작거나 같다.</c:if></div>
		<h4>다중조건비교 : choose ~ when ~ otherwise</h4>
		<pre>
			사용법: 
			< c : choose>
				< c : when test="조건식1">처리할 내용1< /c : when>
				< c : when test="조건식1">처리할 내용2< /c : when>
				< c : when test="조건식1">처리할 내용3< /c : when>
				~~ ~~ ~~
		...
		<c:forEach var="card2" items="${cards2}" varStatus="st">
		...
		<c:forEach var="i" begin="2" end="6">
			${i-1} <img alt="" src="/springGroup/images/${i}.jpg" width="150px" height="100px">
		</c:forEach>
		<br/>
		
		<c:set var="i" value="2/3/4/5/6" />
		<c:forTokens var="card" items="${i}" delims="/" varStatus="st">
			${card-1}
			<img alt="" src="/springGroup/images/${card}.jpg" width="150px" height="100px">
	...
	<div class="border p-3">
		<form action="${pageContext.request.contextPath}/j0807/JSTLtest1_1" name="myform" method="post">
			<div>성명
				<input type="text" id="name" name="name" value="홍길동" class="form-control" />
			</div>
			<div>나이
				<input type="number" id="age" name="age" value="10" class="form-control" />
			</div>
			 <div class="mb-3">성별 &nbsp;&nbsp;
...
<p><br/></p>
	=====>>> URL이동 : 자바크스립트? location.href=''; ==>> redirect ulr = '경로'
