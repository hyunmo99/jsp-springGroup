...
--1번이상 구매한 책들의 정보?(구매한 날짜순 내림차순)
select *from orders;
select *from orders order by orderdate desc;

--1번이상 구매한 책들의 정보?(구매 날짜순 내림차순) 중복된 책들은 배제(1번 출력) -distinct, GROUP BY
select distinct bookid, orderid, orderdate, concat(year(orderdate), '-', month(orderdate), '-', day(orderdate)) from orders orders order by orderdate desc;
select * from orders GROUP BY bookid;
select * from orders GROUP BY bookid order by orderdate desc;


--1번 이상 구매한 사람드의 정보를 구매테이블에서 확인?
...
select o.*, b.bookname from orders o inner join books b on o.bookid = b.bookid;

--1번이상 구매한 책들의 정보?(주문정보, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid;

--1번이상 구매한 책들의 정보?(주문정보, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(최근에 구매한 책 3원만 출력)
select o.orderid, o.orderdate, b.bookname, b.price, o.saleprice from orders o, books b where o.bookid = b.bookid limit 3;

--1번이상 구매한 책들의 정보?(주문정보, 주문날짜, 책이름, 정가, 세일가) - 주문일자 내림차순(중복된 책은 1회만 출력)
...
--책을 구입한 사람들의 이름/책이름/구매가격/구매날짜?(단 구매가자 구매한 책의 관수와 함께 출력)
select c.customer, count(o.custid), b.bookname, o.saleprice, o.orderdate 
	from orders o, books b, customers c
	where o.bookid = b.bookid and o.custid = c.custid
	group by c.customer;	
	
--책을 구입한 사람들의 이름/책이름/구매가격/구매날짜?(단 구매가자 구매한 책의 관수, 구매총금액, 구매 평균과 함께 출력)
...
-- 2025년에 최고로 많이 팔은 책의 이름과 출판사명 출력?
select b.bookname, b.publisher, count(o.bookid)
	from orders o, books b 
	where o.bookid = b.bookid and year(o.orderdate)='2025'
	group by b.bookname;
	
/*---------------------------------------------------------------*/
--서브쿼리(Sub Query) : 하나의 SQL문장안에서 또 다른 SQL문을 포함시키는 것
/*
 : 서브쿼리는 괄호()로 묶어준다.
...
--'대한미디어'나 '삼성당'에서 출판한 도서를 구매한 고객의 이름?(서브쿼리를 사용할 것)
...
--1. '손흥민이' 구매한 도서의 이름, 정가, 구매가격, 할인받은 금액(정가와 판매가격의 차)를 출력하시오.
select b.bookname, format(b.price, 0), format(o.saleprice, 0), format(b.price-o.saleprice, 0) 
	from orders o join books b on o.bookid = b.bookid join customers c on o.custid = c.custid 
	where c.customer='손흥민';

	
--2. 고객별로 주문한 도서의 총수량과 총팬매액을 구하시오. 이때 고객 아이디도 함께 출력하시오.
select o.custid, c.customer, count(o.custid), format(sum(o.saleprice), 0) 
	from orders o join  customers c on o.custid = c.custid 
	group by o.custid; 
	
--3. 도서의 가격과 판매가격의 차이가 가장 많은 주문을 출력하시오.
select *from orders o join books b on o.bookid = b.bookid order by (b.price-o.saleprice) desc limit 1;

--4. 고객별 평균 구매액을 구하시오. 단 출력은 고객명, 평균 구매액을 출력하시오.
select c.customer, format(avg(o.saleprice), 0) from orders o join customers c on o.custid = c.custid group by c.customer;

--5. '2025년 5월 1일' 이후부터 최근까지 판매도니 책의 이름과 판매금액을 출력하시오.