...
--insert into test4 values (default, '다리미', 300, 'JH001');
select * from test4; 
/*
 * 외래키(foreign key) : 하나의 테이블에서 다른테이블의 정보를 찾기위해서는 테이블 서로간의 관계설정을 하여햐 한다.
 * 이때 주테이블의 키는 중복될 수 없는 키(primary key, unique key)가 되어야 되고, 관계설정 맺은 테이블의 필드(키)를 외래키라고 한다.
 */
create table test5 (
	idx int not null auto_increment primary key,
	name varchar(20) not null,
...
--insert into test values (default, '홍길동', 23, 'N002');
insert into test5 values (default, '홍길동', 23, 'N001');
...
select *from student where code=102;
...
--springgroup 데이터베이스의 student테이블의 foreign key를 삭제?
ALTER TABLE student DROP FOREIGN KEY student_ibfk_1;

/*---------------------------활용(JOIN설정)-------------------------*/
-- Inner JOIN = 일반JOIN = 내부조인 = 교집합
...
-- Left Join = Left Outer Join : 왼쪽테이블이 기준이고, 오른쪽 테이블을 조합한다. (즉, 왼쪽테이블의 모든 필드가 출력되게 한다.)
...
-- Right Join = Right Outer Join : 왼쪽테이블이 기준이고, 오른쪽 테이블을 조합한다. (즉, 오늘쪽테이블의 모든 필드가 출력되게 한다.)
...
-- 합집합 : 완전 외부조인(Full Outer Join) - MySQL에서 지원하지 않는다.
-- SELECT * FROM hakkwa h FULL OUTER JOIN student s;
-- MySQL에서는 union을 이용하여 합잡합의 개념으로 사용한다. -Merge(병합)
