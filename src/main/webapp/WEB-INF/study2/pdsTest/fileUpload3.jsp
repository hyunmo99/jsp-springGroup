...
<head>
  ...
  <script>
  	...
  	function fCheck() {
  		...
  		else if(ext != 'jpg' && ext != 'gif' && ext != 'png' && ext != 'zip' && ext != 'hwp' && ext != 'doc' && ext != 'ppt' && ext != 'pptx' && ext != 'pdf' && ext != 'text') {
  	...
  	function fileBoxAppend() {
  		...
  		fileBox +='<input type="button" value="삭제" onclick="deleteBox('+cnt+')" class="btn btn-danger" />';
  		fileBox +='</div>';
...
<div class="container">
	...
	<form name="myform" method="post" action="FileUploadOk3.st" enctype="multipart/form-data">
		<div>
			<input type="button" value="파일박스추가" onclick="fileBoxAppend()" class="btn btn-success mb-2" />
			<input type="file" name="fName1" id="file1" multiple class="form-control mb-3" />
		</div>
		<div id="fileBox"></div>
		<div>
