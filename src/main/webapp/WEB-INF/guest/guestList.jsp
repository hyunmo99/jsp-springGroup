...
<!-- https://www.w3schools.com/bootstrap5/bootstrap_pagination.php 숙제 -->
<!-- 블록페이지 시작 -->
	<div class="text-center">
		<ul class="pagination justify-content-center ">
		  <c:if test="${pag > 1}">
		  	<li class="page-item"><a href="GuestList.gu?pag=1" class="page-link">첫페이지</a></li>
		  </c:if>
		  <c:if test="${curBlock > 0}">
		  	<li class="page-item"><a href="GuestList.gu?pag=${(curBlock-1)*blockSize + 1}" class="page-link">이전블록</a><li class="page-item">
		  </c:if>
		  <c:forEach var="i" begin="${(curBlock*blockSize)+1}" end="${(curBlock*blockSize)+blockSize}" varStatus="st">
		  	<c:if test="${i <= totPage && i == pag}">
		  		<li class="page-item"><a href="GuestList.gu?pag=${i}" class="page-link"><font color="red"><b>${i}</b></font></a></li>
		  	</c:if>
		  	<c:if test="${i <= totPage && i != pag}">
		  		<li class="page-item"><a href="GuestList.gu?pag=${i}" class="page-link">${i}</a></li>
		  	</c:if>
		  </c:forEach>
		  <c:if test="${curBlock < lastBlock}">
		  	<li class="page-item"><a href="GuestList.gu?pag=${(curBlock+1)*blockSize + 1}" class="page-link">다음블록</a></li>
		  </c:if>
		  <c:if test="${pag < totPage}">
		  	<li class="page-item"><a href="GuestList.gu?pag=${totPage}" class="page-link">마지막페이지</a></li>
		  </c:if>
		</ul>
	</div>
<!-- 블록페이지 끝 -->
